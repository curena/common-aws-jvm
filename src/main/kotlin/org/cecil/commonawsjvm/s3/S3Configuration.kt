...
import org.cecil.commonawsjvm.generic.AwsCredentialConfiguration
...
class S3Configuration {
    ...
    /**
     * Creates an Amazon S3 Client using the default credentials provider chain.
     * See: [Default credentials provider chain](https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/credentials-chain.html)
     */
    @Bean
    ...
    fun staticCredentialsClient(staticCredentialsProvider: StaticCredentialsProvider): S3Client {
        return S3Client.builder()
            .credentialsProvider(staticCredentialsProvider).region(Region.of(region)).build()
    }
}