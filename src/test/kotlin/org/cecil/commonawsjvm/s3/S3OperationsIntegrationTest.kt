...
class S3OperationsIntegrationTest {
    private final var localstackImage: DockerImageName = DockerImageName.parse("localstack/localstack:latest")
    ...
    fun should_list_buckets() {
        ...
        assertNotNull(s3Operations.listBuckets().buckets().first { bucket -> bucket.name().equals("foobar") })
    }
